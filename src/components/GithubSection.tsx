...
type Repo = { id: number; name: string; description: string | null; language: string | null; stargazers_count: number; updated_at: string; html_url: string; fork: boolean };
...
function ContributionGrid({ calendar }: { calendar: Contributions }) {
  ...
  return <div className="contribution-grid" role="img" aria-label={`${calendar.totalContributions} GitHub contributions in the last 12 months`}>
    <div className="contribution-grid__heading"><span className="mono">365 DAYS OF BUILDING</span><strong>{calendar.totalContributions} CONTRIBUTIONS</strong></div>
...
export function GithubSection({ username }: { username: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setRepos((await reposResponse.json()).filter((repo: Repo) => !repo.fork).slice(0, 4));
  ...
  useEffect(() => {
    if (!configured) return;
    const abort = new AbortController();
    const loadCalendar = () => fetch("/api/github/contributions", { signal: abort.signal })
  ...
  return <section id="github" className="github section-shell"><div className="section-label mono">OPEN SOURCE / GITHUB <span>CODE ACTIVITY / LAST 12 MONTHS</span></div><div className="github__intro"><h2>Building in<br /><em>the open.</em></h2><p>Public GitHub information loads from the GitHub API. Add your username in <code>src/data/portfolio.ts</code> to connect this section.</p></div>{!configured ? <div className="github__notice">GITHUB API WAITING FOR [GITHUB_USERNAME]</div> : error ? <div className="github__notice">GitHub is unavailable right now. The rest of the portfolio remains available.</div> : <><div className="github-stats">{[[profile?.public_repos, "PUBLIC REPOSITORIES"], [profile?.followers, "FOLLOWERS"], [profile ? stars : undefined, "STARS / LOADED REPOS"]].map(([value, label]) => <div key={String(label)}><strong>{value ?? "—"}</strong><span>{label}</span></div>)}</div>{calendar ? <ContributionGrid calendar={calendar} /> : <div className="contribution-fallback"><span className="mono">CONTRIBUTION GRAPH</span><p>{contributionError ? "Local contribution API unavailable. Add GITHUB_TOKEN to .env.local, then restart npm run dev." : "LOADING CONTRIBUTION CALENDAR…"}</p></div>}<div className="repo-list">{repos.length ? repos.map(repo => <a key={repo.id} href={repo.html_url} target="_blank" rel="noreferrer"><div><small>{repo.language || "CODE"}</small><h3>{repo.name}</h3><p>{repo.description || "No repository description."}</p></div><div className="repo-list__meta">★ {repo.stargazers_count} / {new Date(repo.updated_at).getFullYear()} <ArrowUpRight size={17} /></div></a>) : <div className="repo-skeleton">LOADING PUBLIC REPOSITORIES…</div>}</div></>}</section>;
}
